...
-   **Tasks:**
    -   [ ] **#134 - Add `notification` Module:** Create a new `src/notification` module with `mod.rs`.
    -   [ ] **#135 - Define `Alert` Event Bus in `main`:** In `main.rs`, create a `tokio::sync::broadcast::channel<Alert>`.
    -   [ ] **#136 - Publish Alerts from `app::run`:** Pass the `Sender` end of the channel to `app::run` and call `sender.send(alert)` after an alert is created. Handle potential send errors by logging them.
    -   [ ] **#137 - Create `LoggingSubscriber`:** In the `notification` module, create a `LoggingSubscriber` task that takes a `Receiver`, loops, and logs each received alert.
    -   [ ] **#138 - Add Configuration:** Add `--enable-notifications` flag and `CERTWATCH_ENABLE_NOTIFICATIONS` env var. In `main.rs`, only spawn the subscriber and pass the sender to `app::run` if this is enabled.
    -   [ ] **#139 - Write Pipeline Integration Test:** Create a new integration test that enables notifications, runs the app, and asserts that the expected alert information appears in the logs.
...
-   **Tasks:**
    ...
    -   [ ] **#141 - Add Slack Configuration:** Add `--slack-webhook-url`, `--slack-batch-size`, and `--slack-batch-timeout` arguments and corresponding env vars.
    ...
    -   [ ] **#144 - Test `SlackClient`:** Unit test the `SlackClient` using `wiremock-rs`. Test for successful posts, 500 errors, and network timeouts.
