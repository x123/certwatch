...
async fn test_tsv_enrichment_provider() {
    ...
    let data1 = result1.asn_info.expect("Should find ASN for ip1");
    ...
    let data_start = result_start.asn_info.expect("Should find ASN for ip_start");
    ...
    let data_end = result_end.asn_info.expect("Should find ASN for ip_end");
    ...
    let data2 = result2.asn_info.expect("Should find ASN for ip2");
    ...
    assert!(result3.asn_info.is_none());
    ...
    assert!(result4.asn_info.is_none());
}